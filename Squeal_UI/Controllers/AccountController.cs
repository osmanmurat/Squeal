        ...
        public IActionResult Profile(ProfileViewModel model)
        {
            try
            {
                ...
                if (model.SelectedPhoto != null &&
                    model.SelectedPhoto.ContentType.Contains("image") && model.SelectedPhoto.Length > 0)
                {
                    string fileName = $"{model.SelectedPhoto.FileName.Substring(0, model.SelectedPhoto.FileName.IndexOf('.'))}-{Guid.NewGuid().ToString().Replace("-", "")}";

                    string uzanti = Path.GetExtension(model.SelectedPhoto.FileName);
                    ...
                    if (!Directory.Exists(directoryPath)) // Dosya yoksa oluştur.
                        Directory.CreateDirectory(directoryPath);

                    using var stream = new FileStream(path, FileMode.Create);

                    model.SelectedPhoto.CopyTo(stream);
                    user.ProfilePicture = $"/ProfilePictures/{fileName}{uzanti}";
                    ViewBag.ProfilePageMsg = "Profil resmi yüklendi.";
                }

                else if (model.SelectedPhoto != null &&
                    !model.SelectedPhoto.ContentType.Contains("image"))
                {
